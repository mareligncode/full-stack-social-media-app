...
const PostCard = (props) => {
  ...
  // Use optional chaining and fallback for poster
  const posterUsername = postData.poster?.username || "Anonymous";
  const isAuthor = user && user.username === posterUsername;
      ...
      <Box className={preview}>
        <HorizontalStack spacing={0} alignItems="initial">
          <Stack
            ...
            alignItems="center"
            spacing={1}
            sx={{ backgroundColor: "grey.100", width: "50px", padding: theme.spacing(1) }}
          >
          ...
          <PostContentBox clickable={preview} post={post} editing={editing}>
            <HorizontalStack justifyContent="space-between">
              <ContentDetails
                username={post.poster?.username || "Anonymous"} // Safe access here
                ...
                {user && (isAuthor || user.isAdmin) && preview !== "secondary" && (
                  <HorizontalStack>
                    ...
                    <IconButton disabled={loading} size="small" onClick={handleDeletePost}>
                      {confirm ? <AiFillCheckCircle color={theme.palette.error.main} /> : <BiTrash color={theme.palette.error.main} />}
                    </IconButton>
                  </HorizontalStack>
                )}
              </Box>
            ...
            <Typography variant="h5" gutterBottom sx={{ overflow: "hidden", mt: 1, maxHeight: 125 }} className="title">
              {post.title}
            </Typography>
            ...
            {post.mediaType !== "none" && post.mediaUrl && (
              <Box sx={{ mt: 2, mb: 1 }}>
                {post.mediaType === "image" ? (
                  <CardMedia component="img" image={post.mediaUrl} alt="Post image" sx={{ maxHeight: 400, objectFit: "contain", borderRadius: 1 }} />
                ) : post.mediaType === "video" ? (
                  <CardMedia component="video" controls src={post.mediaUrl} sx={{ maxHeight: 400, borderRadius: 1 }} />
                ) : null}
              </Box>
            )}
